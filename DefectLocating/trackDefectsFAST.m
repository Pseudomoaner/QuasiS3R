...
%         procDefTracks(i).phi = procDefTracks(i).phi + (floor(rand(size(procDefTracks(i).phi))*3)-1)*120; %Randomise orientation in 120 degree increments at every timepoint
        procDefTracks(i).phi = procDefTracks(i).phi + floor((rand(1)*3)-1)*120; %Randomise orientation in 120 degree increments at a single timepoint and then keep orientation fixed
