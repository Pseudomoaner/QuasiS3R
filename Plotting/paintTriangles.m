function outImg = paintTriangles(inImg,xs,ys,thetas,edgeLens,intensities,pxSize,boundaryConds,width,height)
...
for i = 1:size(xs,1)
    ...
    xRot1 = cosd(theta) * xGrid - sind(theta) * yGrid + offsetPx;
    xRot2 = cosd(theta + 120) * xGrid - sind(theta + 120) * yGrid + offsetPx;
    xRot3 = cosd(theta - 120) * xGrid - sind(theta - 120) * yGrid + offsetPx;
    
    triImg = and(and(xRot1 > 0,xRot2>0),xRot3>0);
