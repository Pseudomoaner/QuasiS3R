function expandedProfile = makeExpandedRodProfileTracks(procTracks,cInd,tInd,width,height,pxSize)
...
xPx = round(procTracks(cInd).x(tInd)/pxSize);
yPx = round(procTracks(cInd).y(tInd)/pxSize);
majLenPx = (procTracks(cInd).majorLen(tInd)+extraRim)/(2*pxSize);
minLenPx = (1+extraRim)/(2*pxSize); %2x actual rod width
phi = rad2deg(procTracks(cInd).phi(tInd));
